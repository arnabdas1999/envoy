...
```bash
npx envoy init        # link your project in 30 seconds
...
The entire stack is JavaScript and deploys to Vercel with no separate backend process.
...
│                                 │      │                                   │
│  envoy push ──HTTPS ciphertext─►│      │  Auth: magic link + JWT           │
│  envoy pull ◄──────ciphertext───│      │  Rate limiting: per-IP            │
...
The CLI calls `https://useenvoy.dev/v1/*`. The Next.js app rewrites those paths to `/api/v1/*` routes, so the same deployment serves both the web UI and the API.
...
```
envoy/
├── envoy-cli/               Node.js CLI (published to npm)
│   ├── src/
...
│   │   ├── rate-limit.ts    In-memory rate limiter (Upstash optional)
...
Creates `.envoy.json` in the project root — this file is safe to commit. It contains workspace/project slugs and IDs, no secrets.
...
- name: Pull secrets
  ...
  env:
    ...
    ENVOY_API_URL: https://useenvoy.dev
...
The web dashboard (`https://useenvoy.dev`) gives your team visibility without the CLI:
...
# Edit .env.local — set POSTGRES_URL, JWT_SECRET at minimum
...
export ENVOY_API_URL=http://localhost:3000
```
...
| `RESEND_API_KEY` | [Resend](https://resend.com) API key — use `re_test_...` to log links locally |
| `RESEND_FROM` | Sender address for auth emails |

---

## Deploying to Vercel

1. Fork the repo and import `envoy-web/` into a new Vercel project.
2. Add **Vercel Postgres** from the Storage tab — this sets `POSTGRES_URL` automatically.
3. Run the schema once:
   ```bash
   psql "$POSTGRES_URL" -f envoy-web/supabase/migrations/001_initial_schema.sql
   ```
...
5. Deploy. The CLI's default API URL (`https://useenvoy.dev`) maps to your deployment.
...
| `auth_tokens` | Magic link tokens + long-lived CLI tokens (hashed, scoped, expiring) |
...
POST  /v1/auth/cli-token                   Issue 90-day CLI token
POST  /v1/auth/tokens/revoke               Revoke a token
...
GET   /v1/workspaces/:id/audit             Audit log (paginated)
...
GET   /v1/environments/:id                 Environment detail + ETag
DELETE /v1/environments/:id               Delete environment
GET   /v1/environments/:id/secrets         Pull all secrets (ciphertext)
PUT   /v1/environments/:id/secrets         Push all secrets (If-Match ETag required)
...
The cryptography is solid (AES-256-GCM with HKDF, AAD context binding, random IVs per encryption). The API has rate limiting, ETag concurrency control, soft deletes, and an immutable audit trail. That said, this is a v1 — start with non-production environments and review the code before trusting it with critical secrets.

**Self-hosting?**
Clone the repo, run `docker compose up -d`, apply the migration, and run `npm run dev` in `envoy-web/`. Set `ENVOY_API_URL` to your host. See [Local development](#local-development).
...
```bash
# Run tests
cd envoy-cli && npm test        # 16 unit tests
